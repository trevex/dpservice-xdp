...
def sudo_run(cmd):
	"""Run a command that requires root privileges (sysctl, ip link, etc.)."""
	print(cmd)
	return subprocess.check_output(["sudo"] + shlex.split(cmd))
...
def interface_init(interface, enabled=True):
	if interface.startswith((pf_tap_pattern, vf_tap_pattern, pf_tap_pattern_b, vf_tap_pattern_b)):
		sudo_run(f"sysctl net.ipv6.conf.{interface}.disable_ipv6=1")
	if enabled:
		sudo_run(f"ip link set dev {interface} up")
